/**
 * Patient-facing pathway list (landing + consultation header).
 * Keep codes aligned with backend/data/pathways/*.json filenames.
 */
...
export const PATHWAY_LABELS: Record<string, string> = Object.fromEntries(
  PATIENT_PATHWAYS.map((p) => [p.code, p.label]),
);
